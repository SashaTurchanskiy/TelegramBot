...
public class Bot  extends TelegramLongPollingBot {
    ...
    News newsCollection = new News();
    FeedMap feedMap = new FeedMap();
    HashMap<String, String> newURL = (HashMap<String, String>) feedMap.getNewsURL();
    LinkedList<String> news = newsCollection.getNews(newURL.get(rssfeed));
        ...
        if(msg.contains("Давай новини")|| msg.contains("Новини")|| msg.contains("новини"))
        {
            for (int i=1; i <= amountNews; i++)
            {
                if(news.size()!=0) {
                   ...
                   news = newsCollection.getNews(newURL.get(rssfeed));
                }
            }
    ...
    public void sendInlineKeyBoardMessageFeed() {
        SendMessage message = new SendMessage().setChatId(chatID).setText("Виберіть сервіс з якого ви будете отримувати новини.");
    ...
    public void sendInlineKeyBoardMessageAmount() {
        SendMessage message = new SendMessage().setChatId(chatID).setText("Виберіть кількість новин ,які ви будете отримувати за один раз.");
        InlineKeyboardMarkup markupInline = new InlineKeyboardMarkup();
        List<List<InlineKeyboardButton>> rowsInline = new ArrayList<>();
        List<InlineKeyboardButton> rowInline1 = new ArrayList<>();
        List<InlineKeyboardButton> rowInline2 = new ArrayList<>();
    ...
    private void callDataEquals(String call_data) {
        for (HashMap.Entry<String, String> entry : newURL.entrySet()) {
